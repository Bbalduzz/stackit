...
class BadgeDemo:
    ...
    def setup_menu(self):
        ...
        alerts_item = stackit.MenuItem(
            title="Alerts", callback=lambda s: print("Viewing alerts...")
        )
        alerts_item.set_badge(stackit.BadgeType.ALERTS, count=7)
        ...
        self.dynamic_item = stackit.MenuItem(
            title="Notifications", callback=self.toggle_badge
        )
        ...
        toggle_item = stackit.MenuItem(
            title="Toggle Badge", callback=self.toggle_badge, key_equivalent="t"
        )
        ...
        submenu_with_badges = [
            stackit.MenuItem(
                title="Inbox", badge="new-items", callback=lambda s: print("Inbox")
            ),
            stackit.MenuItem(
                title="Spam", badge="alerts", callback=lambda s: print("Spam")
            ),
            "separator",
        ...
        mail_item = stackit.MenuItem(
            title="Mail", submenu=submenu_with_badges, badge="updates"
        )
    ...
    def toggle_badge(self, sender):
        ...
        if self.badge_enabled:
            self.dynamic_item.set_badge(stackit.BadgeType.NEW_ITEMS, count=3)
