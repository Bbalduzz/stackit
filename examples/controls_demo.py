...
class ComprehensiveDemo:
    def __init__(self):
        ...
        self.app = stackit.StackApp(
            ...
            icon=stackit.SFSymbol(
                "sparkles", rendering="hierarchical", color="#FF6B35"
            ),
        )
    ...
    def setup_menu(self):
        ...
        # About menu item
        about_item = stackit.MenuItem(title="About StackIt", callback=self.show_about)
        ...
        self.notifications_item = stackit.MenuItem(
            title="Notifications", callback=self.show_notifications
        )
        ...
        updates_item = stackit.MenuItem(
            title="Updates Available", callback=self.check_updates, badge="updates"
        )
    ...
    def create_controls_panel(self):
        ...
        header = stackit.hstack(
            [
                stackit.image(
                    stackit.SFSymbol("slider.horizontal.3", rendering="hierarchical")
                ),
        ...
        progress_block = stackit.block(
            stackit.vstack(
                [
                    stackit.label(
                        "Progress Indicators", bold=True, color="#4ECDC4", width=150
                    ),
                    # Linear progress bars
                    stackit.hstack(
                        [
                            stackit.label("Loading:"),
                            stackit.progress_bar(
                                value=0.65, show_text=True, dimensions=(150, 20)
                            ),
                        ]
                    ),
                    stackit.hstack(
                        [
                            ...
                            stackit.progress_bar(
                                indeterminate=True, dimensions=(150, 20)
                            ),
                        ]
                    ),
                    # Circular progress
                    stackit.hstack(
                        [
                            ...
                            stackit.circular_progress(
                                indeterminate=True, dimensions=(20, 20)
                            ),
                            stackit.spacer(),
                        ]
                    ),
        ...
        # ========== Media Block (Video & Map) ==========
        media_block = stackit.block(
            stackit.vstack(
                [
                    stackit.label("Media Controls", bold=True, color="#FFD93D"),
                    # Video player
                    stackit.label("Video Player:", font_size=11, color="#888888"),
                    stackit.video(
                    ...
                    stackit.separator(),
                    # Map view
                    stackit.label("Map View:", font_size=11, color="#888888"),
                    stackit.map_view(
                        latitude=37.7749,
                        longitude=-122.4194,
                        zoom=0.05,
        ...
        button_section = stackit.vstack(
            [
                ...
                stackit.radio_group(
                    ...
                    spacing=4.0,
                    callback=lambda s: print(f"Radio selected: {s.title()}"),
                ),
            ]
        )
    ...
    def create_dynamic_section(self):
        ...
        dynamic_layout = stackit.hstack(
            [
                stackit.image(
                    stackit.SFSymbol(
                        "clock.fill", rendering="hierarchical", color="#4ECDC4"
                    )
                ),
                self.timer_label,
                stackit.spacer(),
    ...
    def create_submenus(self):
        ...
        tools_submenu = [
            stackit.MenuItem(
                title="Calculator", callback=lambda s: print("Calculator")
            ),
            stackit.MenuItem(title="Terminal", callback=lambda s: print("Terminal")),
            "separator",
            stackit.MenuItem(
                title="Activity Monitor", callback=lambda s: print("Activity Monitor")
            ),
        ]
        ...
        export_submenu = [
            stackit.MenuItem(
                title="Export as PDF", callback=lambda s: print("Export PDF")
            ),
            stackit.MenuItem(
                title="Export as PNG", callback=lambda s: print("Export PNG")
            ),
            stackit.MenuItem(
                title="Export as SVG", callback=lambda s: print("Export SVG")
            ),
        ]
        ...
        file_submenu = [
            stackit.MenuItem(
                title="New File",
                callback=lambda s: print("New File"),
                key_equivalent="n",
            ),
            stackit.MenuItem(
                title="Open...", callback=lambda s: print("Open"), key_equivalent="o"
            ),
            "separator",
            stackit.MenuItem(title="Export ▶", submenu=export_submenu),
            "separator",
            stackit.MenuItem(
                title="Close", callback=lambda s: print("Close"), key_equivalent="w"
            ),
        ]
        ...
        view_submenu = [
            stackit.MenuItem(
                title="Zoom In", callback=lambda s: print("Zoom In"), key_equivalent="+"
            ),
            stackit.MenuItem(
                title="Zoom Out",
                callback=lambda s: print("Zoom Out"),
                key_equivalent="-",
            ),
            "separator",
            view_custom_item,
            ...
            submenu=[
                stackit.MenuItem(
                    layout=stackit.hstack(
                        [
                            stackit.image(
                                "https://i.redd.it/y0eiummi1irf1.jpeg",
                                width=200,
                                border_radius=8,
                            )
                        ]
    ...
    def notifications_toggled(self, sender):
        ...
        if not self.notifications_enabled:
            ...
            # Restore badge
            self.notifications_item.set_badge(
                "new-items", count=self.notification_count
            )
    ...
    def saveAction_(self, sender):
        ...
        stackit.notification(
            "Settings Saved", "Your preferences have been saved successfully."
        )
    ...
    def show_preferences(self, sender):
        ...
        stackit.alert(
            "Preferences", "This is where preferences would be configured.", ok="OK"
        )
    ...
    def show_notifications(self, sender):
        ...
        if self.notification_count > 0:
            ...
            self.notifications_item.set_badge(None)
            stackit.notification(
                "Notifications", "All notifications have been marked as read."
            )
    ...
    def run(self):
        ...
        print("  ✓ Video player with AVKit")
        print("  ✓ Interactive maps with MapKit")
