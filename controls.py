...
def button(
    ...
    Args:
        ...
        target: Target object for the action (legacy, use callback instead)
        action: Action selector string (legacy, use callback instead)
        callback: Python function to call when button is clicked (recommended)
    ...
    Note:
        Using callback is recommended over target/action. If callback is provided,
        it takes precedence over target/action.
    """
    from .sfsymbol import SFSymbol
    ...
    # If callback is provided, use the delegate system; otherwise use target/action
    if callback:
        button = NSButton.buttonWithTitle_target_action_(button_title, None, None)
    else:
